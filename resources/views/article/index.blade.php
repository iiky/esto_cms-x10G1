                                ...
                                @foreach ($articles as $article)
                                    <tr>
                                        ...
                                        @canany(['Article Detail', 'Article Category Update', 'Article Category Delete'])
                                            <td>
                                                @can('Article Detail')
                                                    <a href="{{ route('article.show',$article->slug) }}" class="txt-primary"><i data-feather="eye"></i></a>
                                                @endcan
                                                @can('Article Category Update')
                                                    <a href="{{ route('article.edit',$article->slug) }}" class="txt-info"><i data-feather="edit-3"></i></a>
                                                @endcan
                                                @can('Article Category Delete')
                                                    <form method="post" action="{{ route('article.update',$article->slug) }}" id="form-delete-{{ $loop->iteration }}" class="d-inline">
                                                        @csrf
                                                        @method('delete')
                            ...
                            <div class="left-header col horizontal-wrapper">
                                <ul class="horizontal-menu">
                                    <li class="mega-menu outside">
                                        <a class="nav-link" href="{{ route('article.create') }}"><i data-feather="plus"></i><span>Create New Article Categories</span></a>
                                    </li>
                                </ul>
                            </div>
                        </div>
