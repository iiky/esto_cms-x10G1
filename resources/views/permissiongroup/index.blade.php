                                        ...
                                        @canany(['Permission Group Update', 'Permission Group Delete'])
                                            <td>
                                                @can('Permission Group Update')
                                                    <a href="{{ route('permissiongroup.edit',$permissiongroup->id) }}" class="txt-info"><i data-feather="edit-3"></i></a>
                                                @endcan
                                                @can('Permission Group Delete')
                                                    <form method="post" action="{{ route('permissiongroup.destroy',$permissiongroup->id) }}" id="form-delete-{{ $loop->iteration }}" class="d-inline">
                                                        @csrf
                                                        @method('delete')
                            ...
                            <div class="left-header col horizontal-wrapper">
                                <ul class="horizontal-menu">
                                    <li class="mega-menu outside">
                                        <a class="nav-link" href="{{ route('permissiongroup.create') }}"><i data-feather="plus"></i><span>Create New Permission Group</span></a>
                                    </li>
                                </ul>
                            </div>
                        </div>
