...
if ($('#pageWrapper').hasClass('compact-wrapper')) {
    ...
    jQuery('.submenu-title').click(function () {
        // jQuery(this).removeClass('active').find('div').replaceWith('<div class="according-menu"><i class="fa fa-angle-right"></i></div>');
        // jQuery(this).find('.submenu-content').slideUp('normal');
    ...
    // if ($(window).width() < 992) {
    //     $('.dropdown-menu a').clcik(function (e) {
    //         e.preventDefault();
...
// page active
if ($('#pageWrapper').hasClass('compact-wrapper')) {
    var id = $(".compact-wrapper a.active").prop("id");
    var id_array = id.split('-');
    var id_active = "";
    for ($x = 0; $x <= (id_array.length - 2); $x++) {
        if ($x > 0) {
            id_active += '-'
        }
        id_active += id_array[$x];
        $('#' + id_active).addClass('active');
        $('#' + id_active).find('div').replaceWith('<div class="according-menu"><i class="fa fa-angle-down"></i></div>');
        $('#' + id_active).parents().children('ul').css('display', 'block');
    }
